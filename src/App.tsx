...
export default function App() {
  ...
  const [activeTab, setActiveTab] =
    useState<"editor" | "insights" | "history" | "security">("insights");
  ...
  useEffect(() => {
    const safetyTimer = setTimeout(() => setAuthLoading(false), 1500);
  ...
  const handleDeleteInteraction = async (id: string) => {
    ...
    try {
      ...
      if (currentInteraction?.id === id) setCurrentInteraction(null);
  ...
  const handleSaveSuccess = () => {
    if (user?.uid) loadInteractions(user.uid);
  };
      ...
      <main className="flex-1">
        {activeTab === "insights" && (
          <DecisionDashboard
            ...
            displayName={user.displayName || "Your"}
          />
        )}
        {activeTab === "editor" && (
          <JournalEditor
