...
import { Sparkles, ShieldCheck, LogOut, BookOpen, Clock, Lock } from "lucide-react";
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const displayName = user?.displayName || (user?.isAnonymous ? "Guest Explorer" : user?.email?.split("@")[0] || "Authenticated User");
  const email = user?.email || (user?.isAnonymous ? "Temporary Guest Session" : "Google Account");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-400 hidden sm:block">
              Private Journal & Thought Reflection
            </p>
          </div>
        </div>
        ...
        <nav className="flex items-center gap-1 bg-stone-950/60 p-1 rounded-xl border border-stone-800/80">
          ...
          >
            ...
            <span className="hidden md:inline">Security & Cloud</span>
          </button>
        </nav>
